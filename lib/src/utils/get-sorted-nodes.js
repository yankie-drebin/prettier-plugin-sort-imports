...
var getSortedNodes = function (nodes, options) {
    ...
    if (!importOrder.includes(constants_1.THIRD_PARTY_MODULES_SPECIAL_WORD)) {
        importOrder.unshift(constants_1.THIRD_PARTY_MODULES_SPECIAL_WORD);
    }
    var importOrderGroups = importOrder.reduce(function (groups, regexp) {
        var _a;
        return (__assign(__assign({}, groups), (_a = {}, _a[regexp] = [], _a)));
    }, {});
    var importOrderWithOutThirdPartyPlaceholder = importOrder.filter(function (group) {
        return group !== constants_1.THIRD_PARTY_MODULES_SPECIAL_WORD &&
            group !== constants_1.NEW_LINE_SPECIAL_WORD;
    });
    ...
    var finalNodes = importOrder
        .filter(function (group) {
        return importOrderGroups[group].length ||
            group === constants_1.NEW_LINE_SPECIAL_WORD;
    })
        .reduce(function (acc, group, index) {
        if (group === constants_1.NEW_LINE_SPECIAL_WORD) {
            return __spreadArray(__spreadArray([], acc, true), [constants_1.newLineNode], false);
        }
        ...
        acc.push.apply(acc, sortedInsideGroup);
        if (importOrderSeparation || index === importOrder.length - 1) {
            acc.push(constants_1.newLineNode);
        }
        return acc;
    }, []);
